...
import com.example.absensi.adapter.RiwayatCutiAdapter
...
class RiwayatCutiActivity : AppCompatActivity() {
        ...
        ApiConfig.getApiService().getRiwayatCuti("Bearer $token").enqueue(object : Callback<CutiResponse> {
            override fun onResponse(call: Call<CutiResponse>, response: Response<CutiResponse>) {
                if (response.isSuccessful && response.body() != null) {
                    ...
                    val adapter = RiwayatCutiAdapter(dataCuti)
                    rvRiwayatCuti.adapter = adapter
                } else {
                    Toast.makeText(this@RiwayatCutiActivity, "Gagal mengambil data", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<CutiResponse>, t: Throwable) {
                Toast.makeText(this@RiwayatCutiActivity, "Koneksi Error: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
}